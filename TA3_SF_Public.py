...
Public = parsedLines.filter(lambda x: "No" in x[0])   #keep public school only 

data = Public.map(lambda x: (Vectors.dense(float(x[1])),float(x[2]))).cache()
...
# !spark-submit TA3_SF_Public.py >TA3_SF_Public.txt