...
Public = parsedLines.filter(lambda x: "Yes" in x[0]) 

data = Public.map(lambda x: (Vectors.dense(float(x[1])),Vectors.dense(float(x[2])),Vectors.dense(float(x[3])),float(x[4]))).cache()
...
# !spark-submit TA3_Cost_Public.py > TA3_Cost_Public.txt